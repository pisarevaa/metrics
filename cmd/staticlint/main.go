...
// Доабвляем нужные golang.org/x/tools/go/analysis/passes
var passes = []*analysis.Analyzer{ //nolint:gochecknoglobals // new for task
...
func main() {
	// определяем map подключаемых правил staticcheck
	...
	var selectedAnalyzers []*analysis.Analyzer
	for _, v := range staticcheck.Analyzers {
		// добавляем в массив нужные проверки staticcheck
		if checks[v.Analyzer.Name] {
			selectedAnalyzers = append(selectedAnalyzers, v.Analyzer)
		}
	}

	selectedAnalyzers = append(selectedAnalyzers, passes...)
	selectedAnalyzers = append(selectedAnalyzers, OSExitInMainAnalyzer)

	multichecker.Main(selectedAnalyzers...)
}
